...
class SubnetDevices private constructor() {
    private var noThreads = 100

    private var addresses: ArrayList<String>? = null
    private var devicesFound: ArrayList<Device> = ArrayList()
    ...
    fun setDisableProcNetMethod(disable: Boolean) {
        this.disableProcNetMethod = disable
    }
    ...
    fun findDevices(listener: OnSubnetDeviceFound): SubnetDevices {
        ...
        Thread { // Load mac addresses into cache var (to avoid hammering the /proc/net/arp file when
            ...
            ipMacHashMap = if (disableProcNetMethod) ARPInfo.allIpAndMACAddressesFromIPSleigh else ARPInfo.allIPAndMACAddressesInARPCache
            ...
            for (device in devicesFound) {
                if (device.mac == null && ipMacHashMap!!.containsKey(device.hostAddress)) {
                    device.mac = ipMacHashMap!![device.hostAddress]
                }
            }
    ...
    inner class SubnetDeviceFinderRunnable internal constructor(private val address: String?) : Runnable {
        override fun run() {
            if (cancelled) return

            try {
                val ia = InetAddress.getByName(address)
                val pingResult = Ping.onAddress(ia).setTimeOutMillis(timeOutMillis).doPing()
                if (pingResult.isReachable) {
                    val device: Device = Device(ia)
    ...
    companion object {
        ...
        fun fromLocalAddress(): SubnetDevices {
            val ipv4 = IPTools.localIPv4Address ?: throw IllegalAccessError("Could not access local ip address")
        ...
        fun fromIPAddress(ipAddress: String): SubnetDevices {
            require(IPTools.isIPv4Address(ipAddress)) { "Invalid IP Address" }
            ...
            for (ip in ARPInfo.allIPAddressesInARPCache) {
