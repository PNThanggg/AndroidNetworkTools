...
import com.module.network.tools.portscan.PortScanTCP
import com.module.network.tools.portscan.PortScanUDP
import java.net.InetAddress
import java.net.UnknownHostException
import java.util.concurrent.Executors
import java.util.concurrent.TimeUnit

class PortScan private constructor() {
    ...
    fun setPorts(portString: String?): PortScan {
        ...
        val portSubString = portString.substring(portString.indexOf(":") + 1, portString.length)

        for (x in portSubString.split(",".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray()) {
            if (x.contains("-")) {
                val start = x.split("-".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray()[0].toInt()
                val end = x.split("-".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray()[1].toInt()
                validatePort(start)
                validatePort(end)
    ...
    private inner class PortScanRunnable(
        ...
        override fun run() {
            ...
            when (method) {
                METHOD_UDP -> portScanned(portNo, PortScanUDP.scanAddress(address, portNo, timeOutMillis))
                METHOD_TCP -> portScanned(portNo, PortScanTCP.scanAddress(address, portNo, timeOutMillis))
    ...
    companion object {
         ...
         * @return this object to allow chaining
         */
        private fun onAddress(ia: InetAddress): PortScan {
